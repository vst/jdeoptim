...
public class Strategy3 implements Strategy {
    ...
    public void regenerate(Population population, Problem problem, Objective objective) {
        ...
        UniformIntegerDistribution elementSampling = new UniformIntegerDistribution(this.randomGenerator, 0, population.getDimension() - 1);
        ...
        for (int c = 0; c < population.getSize(); c++) {
            ...
            final int[] randomCandidates = Utils.pickRandom(Utils.sequence(population.getSize()), 2, new int[]{c}, this.randomGenerator);
            ...
            for (int i = 0; i < trial.length; i++) {
                ...
                if (trial[i] < problem.getLower()[i]) {
                    if (this.bounceBack) {
                        trial[i] = problem.getLower()[i] + probability.sample() * (problem.getUpper()[i] - problem.getLower()[i]);
                    }
                    else {
                ...
                if (trial[i] > problem.getUpper()[i]) {
                    if (this.bounceBack) {
                        trial[i] = problem.getUpper()[i] - probability.sample() * (problem.getUpper()[i] - problem.getLower()[i]);
                    }
                    else {
